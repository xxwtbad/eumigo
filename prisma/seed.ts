...
async function main() {
  ...
  // 创建 eumigo 用户
  const eumigoPassword = await bcrypt.hash("eumigo123", 10);
  await prisma.user.upsert({
    where: { username: "eumigo" },
    update: {},
    create: {
      username: "eumigo",
      hashed_password: eumigoPassword,
      nickname: "eumigo",
      is_admin: true,
    },
  });
  ...
  const siteConfigs = [
    ...
    { key: "bio", value: "这里是 玉米狗的小站 的个人博客，记录技术、生活与点滴思考。", description: "个人简介" },
    ...
    { key: "cloud_music_playlist_id", value: "", description: "网易云音乐歌单ID" },
    { key: "cloud_music_ids", value: "", description: "网易云音乐歌曲ID数组（JSON）" },
  ...
  console.log("Seed completed: admin/eumigo users and site configs created.");
}
